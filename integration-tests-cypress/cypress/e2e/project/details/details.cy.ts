  ...
  context('Given a Batmobile Project', () => {
    ...
    context('When we select a succession with no guard Expression', () => {
      ...
      it('Then creating an expression leaves a single Expression value widget', () => {
        ...
        cy.getByTestId('details-expression-value').should('have.length', 1).and('have.text', 'true');
        ...
        cy.getByTestId('edit-sysml-expression-modal').should('not.exist');
        cy.getByTestId('details-expression-value').should('have.length', 1).and('have.text', 'false');
      });
    });
  });
});
