...
There is no easy way for users to differentiate standard libraries from user-imported libraries.
...
It is not possible to import a _Standard Library_: these libraries are already available in every SysON project, and they are the only libraries that can be standard in the context of SysMLv2.
...
- Library versioning is out of scope for this work.
