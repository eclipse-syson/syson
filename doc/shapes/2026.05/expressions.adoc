...
Expressions can appear in several places (non-exhaustive list): successions, transitions, attribute values, constraints, assumptions, and so on.
...
Currently, SysON has poor support for expressions:
...
- in the _Explorer_ view, expressions are exposed as their raw AST, which makes them unusable by end users.
...
Even though valid expressions are stored as modeled ASTs, the natural way for end users to interact with expressions is through their textual representations (for example, `totalMass == sum(partMasses)`). Therefore, we will ensure that:
...
Different UI options have been discussed to allow users to edit the textual representation of expressions:

1. We could leverage the existing "direct edit" support, at least in the _Explorer_ view and on _diagrams_.
However, the UX workflow of simple direct edit is too limited to support validation before applying a change and to keep the user informed about the validity of the entered text.
...
However, the behavior of the existing `Textfield` widget would not fit the intended workflow, as it will try to send its current text to the backend as soon as it loses focus.
...
This would work but would require significant development effort.
Creating such a rich, generic, configurable "source edition widget" could be useful for other cases and might happen later. However, given the appetite for this pitch, this option was rejected for now.

4. We could create a custom UI inside a modal dialog.
It would be very similar to the existing _New object as text_ dialog, but would support only _expressions_ and follow a slightly different workflow.
...
It will not actually edit the model or update the expression.
The backend will return a diagnostic, at the very least a valid/invalid status and, if possible, details about validation errors.
...
Direct edit will be disabled on expression items in the Explorer.
As mentioned above, the direct-edit workflow does not meet the constraint of ensuring expression validity.
...
Instead, a *custom action* in these nodes' context menu will open the dedicated expression editing UI described above.
...
If we decide to allow invalid expressions to be stored in the model, we need a way to persist the user-supplied text for an expression even if it is currently invalid.

For example, if a user enters `totalMass == sum(partMasses` for a constraint (with a missing closing parenthesis), we cannot parse it as a valid model. Instead of creating invalid model content, we will store the raw string `"totalMass == sum(partMasses"`, tell the user about the problem, and allow them to edit the string into a valid one.

Two possible options (there might be more) for storing this raw text:

* Use EMF `EAnnotations`, because the root type `Element` is also an `ecore.EModelElement`. For example:
[source,java]
----
...
* Leverage SysML's notion of _textual representation_ (see § 7.4.3 of the SysML specification) with a custom language (for example, `"syson:user-input:expression"`) to keep syntactically invalid user-supplied text until it is fixed and can be parsed.
...
- When the user edits an expression as text, once the expression has been correctly parsed as a valid model, we will not try to retain any original formatting (for example, whitespace).
