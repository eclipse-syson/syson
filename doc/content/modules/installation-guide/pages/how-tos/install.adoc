...
[CAUTION]
====
If on the same project you plan to have multiple client connections then you need to do a xref:how-tos/install.adoc#Manual_Install[manual installation] instead of a Docker Compose one. 
====
...
To run the application, you'll need to use the Java Runtime Environment (JRE) to execute the pre-build JAR ({product-jar}) you xref:how-tos/install.adoc#Manual_Install[downloaded] earlier.
