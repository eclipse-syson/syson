...
Before manually deploying the database, we recommend using a managed database instance provided by the organization's IT service or through cloud services.
...
To run the application, use the Java Runtime Environment (JRE) to run the prebuilt JAR, {product-jar}, xref:how-tos/install/production_deploy.adoc#download[downloaded] earlier.
