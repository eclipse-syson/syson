...
----
docker run -p 5433:5432 --name {product-lower}-postgres \
                            -e POSTGRES_USER=dbuser \
                            -e POSTGRES_PASSWORD=dbpwd \
                            -e POSTGRES_DB={product-lower}-db \
                            -d postgres
----
...
[source, bash, subs=attributes+]
----
docker run -p 5433:5432 --name {product-lower}-postgres -e POSTGRES_USER=dbuser -e POSTGRES_PASSWORD=dbpwd -e POSTGRES_DB={product-lower}-db -d postgres
----
====
...
[source, bash, subs=attributes+]
----
java -jar path/to/your/{app-name}-YEAR.MONTH.0.jar \
          --spring.datasource.url=jdbc:postgresql://localhost:5433/{product-lower}-db \
...
[TIP]
====
To end your local testing session, confirm you stop the running PostgreSQL Docker container by using the command `docker kill {product-lower}-postgres`.
