...
[NOTE]
====
If a platform and version are not listed on this page, they are not tested, supported with bug fixes, or eligible for assistance.
====
...
* Use *Java 21* to run the {product} Server.
...
Users can interact with the central server through the following web browsers:
...
For optimal performance, we strongly recommend hosting both the server and database on the same physical server to reduce latency between the two components.

=== Network stability

Virtual Private Networks (`VPNs`) are not recommended.
They are a common source of latency, as are other network elements that can drop more-or-less inactive connections.
...
We make no warranty regarding the statements in the <<Deployment recommendations>> section; they are for informational purposes only.
