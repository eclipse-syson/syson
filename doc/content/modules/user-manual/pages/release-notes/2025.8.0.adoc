...
- Fix action "New objects from text" being available on all semantic elements in the Explorer view.
Now the backend may specify on which elements this contextual menu action is available.
The default implementation only enables it on SysML model elements.
