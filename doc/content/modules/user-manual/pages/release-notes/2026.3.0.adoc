...
** Add _attributes_ and _doc_ compartments on _ConnectionDefinition_ graphical nodes.
...
** The graphical edge tools _Become nested Attribute_ is added to `AttributeUsage` when the targeted element is a `Usage` or `Definition`.
** The graphical edge tools _Add as nested Attribute_ is added to all `Usage` and `Definition` that target an `AttributeUsage`.
