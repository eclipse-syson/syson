...
** Leverage the selection dialog to improve the graphical node tools creating a _require_ `ConstraintUsage`, or an _assume_ `ConstraintUsage`, from `RequirementUsage` and `RequirementDefinition` graphical nodes.
...
** When using direct edit on feature elements which have a value expression (e.g. an `Attribute` with a default value defined), the expression part (after the `=` sign) is no longer part of the initial direct edit value, and will be ignored if supplied in the new text value.
...
** It is now possible to create, edit and delete _Expressions_ using their textual syntax from the a diagram.
...
The behavior is the same when invoking the equivalen action from the _Explorer_ or _Details_ view.
+
image::expression-palette-section.png[Expression palette tool section on an `Attribute` in a diagram, width=80%]
** In an _action flow_ compartment, `Succession` edges which have a guard now display the guard condition `Expression` as a _begin label_:
+
image::action-flow-guard-labels.png[Succession edges with visible guard expressions, width=80%]
...
** Display inherited `ItemUsages` in the _items_ compartment, and inherited `ActionUsages` in the _actions_ compartment of `ActionDefinitions`, `ActionUsages`, `PartUsages`... graphical nodes.
