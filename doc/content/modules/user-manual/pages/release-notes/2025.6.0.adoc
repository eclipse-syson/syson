...
- Prevent `TransitionUsage` edges to be reconnected to another `TransitionUsage`.
In the previous implementation, it was possible during an edge reconnection to drop the source or target to another `TransitionUsage.
The semantic of the `TransitionUsage` being reconnected was changed and the edge was removed from the diagram.
...
- The `ViewUsage#exposedElement` feature is now updated to reflect the contents of the diagram (_General View_,...) on which it is associated.
