...
- In diagrams, a `FeatureValue` connecting two `Features` now displays its _implicit_ `BindingConnectorAsUsage` as a graphical edge.
Note that the _implicit_ `BindingConnectorAsUsage` is not added to the model, but is only represented in the diagram.
