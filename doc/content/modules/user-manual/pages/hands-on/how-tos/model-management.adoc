...
[WARNING]
====
This operation is irreversible. Once completed, there is no way to recover the element.
...
* You can set the cardinality of an element after or before a _typing part_ (`:`), _subsetting part_ (`:>`) or _redefinition part_ (`:>>`) as such both expressions `part p2[4] :> p1` and `part p2 :> p1[4]` are valid and the first element of the expression will have its cardinality updated.
* Typing `someElement:>otherElement` on an element sets _someElement_ as a `Subsetting` of _otherElement_ if both elements are of the same type.
