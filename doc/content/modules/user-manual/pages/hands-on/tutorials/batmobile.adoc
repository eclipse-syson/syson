...
====
+
. Rename the part definition to `Vehicle` using the xref:hands-on/how-tos/model-management.adoc#representation[direct-edit action] or the xref:hands-on/how-tos/model-management.adoc#details[{details} view].
...
. At the end of the previous section, a new `Wheel` part definition has been created in the model.
...
. A new containment edge is created, and the `frontRightWheel` element is moved under `Batmobile` in the model.
...
. Select the `Become nested part` tool.
. A new containment edge is created, and the `bulletProofWheels` element is moved under `BatmobileNG` in the model.
