...
. Set the value of `connectDCPwr::onOffCmd` by editing the item `in onOffCmd` of `connectDCPwr` to `in onOffCmd = produceDirectedLight.onOffCmd`. Do that in the _items_ compartment of the `connectDCPwr` action in the diagram.
