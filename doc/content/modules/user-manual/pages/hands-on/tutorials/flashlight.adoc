...
. Click on the `Manage Visibility -> require constraints` (i.e. the eye icon that will appear when the mouse pointer will be over the `weight` requirement) on the `weight` requirement to display the _require constraints_ compartment on the `weight` requirement.
...
. Open it's palette, and invoke the `New Expression` tool in the `Expression` section.
...
. Use the eye icon on the `produceDirectLight` action to display the `items` compartment.
...
. Right-click on `flashlight`, then select the `Behavior` -> `New Perform`, choose the second option in the selection dialog, then select the action `produceDirectedLight` created earlier.
...
. A new port (a graphical border node) has been created, rename it `inPort`.
. Right-click on `switch`, then select the `Structure` -> `New Port Out`.
. A new port (a graphical border node) has been created, rename it `outPort`.
. Right-click on `flashlight`, then select the `Structure` -> `New Part` to create a nested part.
. Rename it `battery`.
