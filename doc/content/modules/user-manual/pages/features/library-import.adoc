...
With the latter, a copy of the library contents is copied into the project. The copied contents can be used and edited as if they had been created in the project. There is no link between the copied contents and the original library contents, so they cannot be resynchronized (for example, when the library has a new version).
...
If a document from an imported-by-reference library has root elements that are all instances of `LibraryPackage`, then that document is displayed under the 'User libraries' node in the 'Explorer' view.

image::release-notes-publication-import-user-library-with-library-name-and-read-only.png[Imported published library under User libraries, width=50%,height=50%]

Note that imported-by-reference libraries have `library-name@library-version` in their label to quickly identify their containing library.

== Updating the version of a library dependency

The version of a published library used by a project may be updated.
Because published library versions are strings, there is no natural order for library versions.
...
Select the new version of the library, then click on the 'Update' button.
A dialog prompts showing the impact analysis report associated to the library update.
...
Note that this tree shows the same element as the Explorer view with the default filters enabled (it hides the `Membership` elements, the root `Namespace`, and the `MembershipExpose` elements).

The example below shows a library update operation that will:

- Remove the old library dependency (`SysMLv2-Library@v1`)
- Add the new library dependency (`SysMLv2-Library@v2`)
- Remove `partLib` from the `subsettingFeature` in `part1`'s `Subsetting` (because `partLib` was removed between `SysMLv2-Library@v1` and `SysMLv2-Library@v2`)

Note that the first two lines can be expanded to see the complete models of the removed/added libraries.

Clicking on the 'Execute' button will perform the update, including the impacts listed in the dialog. If you don't want to update the library you can close the dialog and your models will remain unchanged.

image::release-notes-library-update-impact-analysis-with-tree.png[Dialog showing the impact analysis report of the 'Update library' action, width=85%,height=85%]
...
Libraries may add, remove, or edit their contents freely between versions, so there is no guarantee that two versions of a library are compatible or even similar.
