...
This view lets you write AQL queries (see the https://eclipse.dev/acceleo/documentation/[AQL documentation] for details) and view their results.
[WARNING]
====
This view comes from https://eclipse.dev/sirius/sirius-web.html[{sw}], the software on which {product} is based.
It will be improved in future releases with new variables and services, but is currently experimental.
====
Three variables are currently available to help write queries:
...
- `editingContext`, which represents the current project.
...
Use this service carefully, as it can retrieve thousands of elements and cause performance issues.
