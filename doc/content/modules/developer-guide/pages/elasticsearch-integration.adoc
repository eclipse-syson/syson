...
When configured, this feature transparently indexes models created and modified by users.
...
Elasticsearch comes with Kibana, a browser-based analytics and search dashboard that lets you visualize and manage your indices, among other features not covered here.
...
Put this certificate somewhere on your computer and use it to test the connection to Elasticsearch.
...
If you never set a custom password, https://docs.oracle.com/en/java/javase/21/docs/specs/man/keytool.html[the default is `changeit`].
...
{product} does not define such repositories, so the scan does not affect the application, but it generates messages that can pollute the backend logs.
...
Each {product} editing context (that is, a projectfootnote:[In some cases, projects can have multiple editing contexts, but this documentation does not cover them.]) has its own index, named `editing-context-<Editing Context ID>`.
...
[NOTE]
====
The Discover view can display only 500 documents. Use the fields in the left panel to filter the results, or see the next section to run queries on an index.
====
...
You can use the query DSL to define more complex queries. For example, you can get all instances of a given type that have a name matching a given pattern:
...
The query above uses a _wildcard_ in the index name, so it queries all indexes that start with `editing-context-`.
