...
==== Use SysON in a downstream application

Add `org.eclipse.syson:syson-starter` to a Spring Boot application's dependencies to use SysON. The starter provides the SysON runtime modules and automatically configures its Spring components.

[source,xml]
----
<dependency>
  <groupId>org.eclipse.syson</groupId>
  <artifactId>syson-starter</artifactId>
  <version>YYYY.M.0</version>
</dependency>
----

Replace `YYYY.M.0` with a released SysON version.
The downstream application remains responsible for its entry point and operational dependencies, such as Spring Boot Actuator.

The starter's `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` file registers `SysONStarterConfiguration`.
Spring Boot loads that configuration when auto-configuration is enabled, including through `@SpringBootApplication`, so downstream applications do not need to import it explicitly.
...
Go to the _Help > Install New Software..._ menu, then add _https://checkstyle.org/eclipse-cs-update-site/_ in the _Work with:_ selection field and press _Enter_.
...
Go to the _Help > Install New Software..._ menu, then select _https://download.eclipse.org/modeling/emf/emf/builds/release/latest/_ in the _Work with:_ selection field and press _Enter_.
...
+
Make sure that the active profile is `SysON`, then click the _Apply_ button.
...
In Eclipse, go to the Git perspective, then click the _Add an existing local Git repository_ button.
...
Click _Finish_. Your projects should now be in the _Package Explorer_ of your _Java_ perspective. In the bottom-right corner of Eclipse, you should see the imported projects being built.
Wait for the end of this task (this could take a while).

===== Post-configuration

Configure Checkstyle to use the {product} Checkstyle configuration.
...
The current Node version to use is 24.20.0.
The current NPM version to use is 11.19.0.
...
* All services in this module must not depend on Spring, Sirius-Web, or diagram- or table-related code.
...
* A service that queries the model belongs in a class named `ModelQueryService`.
* A service that mutates the model belongs in a class named `ModelMutationService`.
* A service that queries the diagram belongs in a class named `DiagramQueryService`.
* A service that mutates the diagram belongs in a class named `DiagramMutationService`.
* A service that queries diagram-element labels belongs in a class named `DiagramQueryLabelService`.
* A service that modifies diagram-element labels belongs in a class named `DiagramMutationLabelService`.
* The same naming convention applies to tables, trees, forms, and representations.
* `XXXQueryService`, `XXXMutationService`, `XXXQueryYYYService`, and `XXXMutationYYYService` classes can be split into smaller classes if they contain too many services.
