...
<2> Define the API endpoint: Specifies the starting point for all REST APIs in {product}, which is `/api/rest/`.
<3> Construct the full API URL: Combines the base host and the {product} API endpoint into a complete URL for API interaction.
...
Once you have configured the {sysmlv2} API access, you can start testing the xref:api-cookbook.adoc#_recipes[recipes].
...
include::project_commit_branch_tag_recipe.adoc[leveloffset=+2]

include::element_create_update_delete_recipe.adoc[leveloffset=+2]

include::element_owned_elements.adoc[leveloffset=+2]

include::queries.adoc[leveloffset=+2]
