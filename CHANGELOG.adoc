...
- [releng] Update to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2026.7.4].
...
- https://github.com/eclipse-syson/syson/issues/2510[#2510] [import] Fix the silent truncation of a SysML file containing a syntax error during a textual import.
The parser used to produce the AST reports neither a non-zero exit code nor anything on its standard error in that case, and prints an AST covering only the part of the file located before the error.
The import used to succeed with an empty report while everything located after the error was missing from the resulting model.
Such a file is now reported as invalid, with the line and the message of each syntax error, and is no longer imported.
