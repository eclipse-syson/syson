...
- Display inherited elements in diagrams' free form compartments like _action flow_, _state transition_ and _interconnection_.

=== Architectural decision records

=== Breaking changes

- [services] The new `addExistingConnectedElements` service accepts a list of semantic elements and a list of selected graphical nodes so that filtering is applied to the whole selection before exposure.
- https://github.com/eclipse-syson/syson/issues/2535[#2535] [services] `org.eclipse.syson.application.services.DetailsViewService` has been removed as part of the form service architecture migration.
Its AQL entry points are now exposed by `org.eclipse.syson.form.services.aql.FormMutationAQLService` for mutations and `org.eclipse.syson.form.services.aql.FormQueryAQLService` for queries.
Downstream code using the former implementation service must migrate to `org.eclipse.syson.form.services.FormMutationElementService` and `org.eclipse.syson.form.services.FormQueryElementService`, according to whether it mutates or queries the form model.
`CoreFeaturesSwitch` and `EStructuralFeatureLabelProvider` have moved from `org.eclipse.syson.application.services` to `org.eclipse.syson.form.services`.
The `getCommentBody`, `getComment`, and `getDocumentation` query APIs are now provided by `org.eclipse.syson.sysml.metamodel.services.MetamodelQueryElementService`.
- https://github.com/eclipse-syson/syson/issues/2537[#2537] [services] `org.eclipse.syson.services.ImportService` has moved to `org.eclipse.syson.sysml.metamodel.services.MetamodelMutationImportService` as part of the metamodel service architecture migration.

=== Dependency update

- [releng] Update to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2026.9.2].
- [releng] Update to https://github.com/spring-projects/spring-boot/releases/tag/v4.1.1[Spring Boot 4.1.1]
- [releng] Update to https://github.com/testing-library/user-event/releases/tag/v14.6.7[@testing-library/user-event 14.6.7]
- [releng] Update to https://github.com/testing-library/react-testing-library/releases/tag/v16.3.3[@testing-library/react 16.3.3]
- [releng] Update to https://github.com/vercel/turborepo/releases#release-v2.10.12[turbo 2.10.12]
- [releng] Update to https://github.com/vitejs/vite/releases/tag/v8.3.0[Vite 8.3.0] and https://github.com/vitejs/vite-plugin-react/releases/tag/plugin-react%406.1.1[@vitejs/plugin-react 6.1.1]
- [releng] Update to https://github.com/nodejs/node/releases/tag/v24.20.0[node.js 24.20.0] and https://docs.npmjs.com/cli/v11/using-npm/changelog#11190-2026-07-28[npm 11.19.0] for our development environment.
- [releng] Update to EMFJson 2.5.4

=== Bug fixes

- https://github.com/eclipse-syson/syson/issues/1074[#1074] [diagrams] Fix available diagram tools on standard diagrams created on a root namespace
- https://github.com/eclipse-syson/syson/issues/2510[#2510] [import] Report syntax errors that may leave an uploaded SysML file partially imported.
The parsed elements are still imported, and the loading report includes the parser diagnostics and a warning about possibly missing elements.
- https://github.com/eclipse-syson/syson/issues/2528[#2528] https://github.com/eclipse-syson/syson/issues/2429[#2429] https://github.com/eclipse-syson/syson/issues/2213[#2213] [export] Export successions with an implicit target as `then <action>;` instead of a dangling `then;`, and export unnamed decision nodes as `decide;`. When the succession has an explicit `first` source, the target is referenced by name (`first start then a2;`) instead of inlined.

=== Improvements

- https://github.com/eclipse-syson/syson/issues/2494[#2494] [test] Improve `SemanticRunnableFactory` by adding an API that emits the appropriate change description for editing context updates in integration tests.
- [cleanup] Use `SemanticRunnableFactory` more widely in integration tests, replacing the `IExecuteEditingContextFunctionRunner` by the `SemanticRunnableFactory`.
- [releng] Remove redundant inherited Maven `groupId` and `version` declarations from backend modules.

=== New features

- [releng] Add the `syson-starter` Maven module so downstream Spring Boot applications can use SysON with a single dependency.
- https://github.com/eclipse-syson/syson/issues/2469[#2469] [diagrams] `New Reference` tools should have a selection dialog.
- https://github.com/eclipse-syson/syson/issues/2467[#2467] [diagrams] Add the _Add existing connected elements_ action to _General View_ diagrams.
The action adds existing project elements connected to the selected element through inbound or outbound relationships.
Elements representable as border nodes or inside any selected node are excluded, including when the corresponding compartment is hidden.
Use the existing content tools to display those elements; existing representations are preserved.
...
- [services] As part of the migration to the new diagram service architecture, the methods that were exposed by `ViewEdgeService` (which has been deleted) are now split between `org.eclipse.syson.diagram.services.aql.DiagramMutationAQLService` for edge reconnection mutation services, `org.eclipse.syson.diagram.services.aql.DiagramQueryAQLService` for diagram graphical query services, and `org.eclipse.syson.model.services.aql.ModelQueryAQLService` for model and metamodel query services.
- [services] As part of the migration to the new diagram service architecture, `ViewCreateService` has been deleted.
Its former AQL entry points are now split between `org.eclipse.syson.diagram.services.aql.DiagramMutationAQLService` for diagram and tool mutations, `org.eclipse.syson.diagram.services.aql.DiagramQueryAQLService` for diagram creation preconditions, tool preconditions, and inherited-members queries, and `org.eclipse.syson.model.services.aql.ModelMutationAQLService` for metamodel initialization.
- [services] `ShowDiagramsInheritedMembersService` has moved from `syson-diagram-common-view` to `syson-diagram-services`.
Downstream code should now depend on the diagram services module for that bean.
- [services] `InheritedCompartmentItemFilterSwitch` has moved from `syson-diagram-common-view` to `syson-diagram-services` as part of the inherited-compartment query migration.
- [releng] Frontend packages published from `frontend` do not provide UMD bundles anymore.
...
- https://github.com/eclipse-syson/syson/issues/2418[#2418] [services] Remove the `unwrapFeature` service since it is not used anymore.
- https://github.com/eclipse-syson/syson/issues/2452[#2352] [diagram] SysON now defines its own `DiagramDefaultToolsFactory` providing show/hide tools.
SysON consumers should use this new service instead of the Sirius Web's DefaultToolsFactory.
- [tests] `org.eclipse.syson.application.controller.explorer.testers.TreeItemContextMenuTester.java` has been removed since it was not used anymore.
Please use `org.eclipse.sirius.components.trees.tests.graphql.TreeItemPaletteExecutor.java` from Sirius Web instead.

=== Dependency update

- [releng] Update to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2026.9.0].
...
- [releng] Downgrade liquibase from 5.0.3 to 4.33.0.
...
- https://github.com/eclipse-syson/syson/issues/2472[#2472] [diagrams] Fix nested-usage edge tools in _General View_ diagrams.
The connector palette now provides exactly one type-specific tool for a Usage target and no such tool for non-Usage targets. Usage sources provide _Add target as nested <type>_ tools; Definition sources provide _Add target as owned <type>_ tools. _Become nested <type>_ tools remain specific to the source usage kind.
- https://github.com/eclipse-syson/syson/issues/2132[#2132] [diagrams] Fix the empty-diagram background remaining visible after creating a namespace import in a _General View_ diagram.
...
- https://github.com/eclipse-syson/syson/issues/2208[#2208] [diagrams] Fix the edge tool creating a `ConnectionUsage` between an `ActionUsage` graphical node and the _done_ `ActionUsage` graphical node.
...
- https://github.com/eclipse-syson/syson/issues/2466[#2466] [diagrams] Fix subclassification edge tool in _General View_ diagrams.
- https://github.com/eclipse-syson/syson/issues/2485[#2485] [diagrams] Fix the _New Derived Requirement_ edge tool creating a derivation between two `RequirementUsage`.
- https://github.com/eclipse-syson/syson/issues/2474[#2474] [diagrams] Do not offer expression tools on inherited border nodes or inherited compartment items.
- https://github.com/eclipse-syson/syson/issues/2464[#2464] [diagrams] Prevent the `Add existing elements` tools from removing the empty diagram background image when no elements are added.
- https://github.com/eclipse-syson/syson/issues/2473[#2473] [services] Fix the expression dialog by automatically import the SI package via `NamespaceImport` when un-prefixed standard units are used in an expression.
The `NamespaceImport` is created under the closest ancestor `Package` from the element holding the expression, or in the root `Namespace` if no ancestor `Package` is found.
- https://github.com/eclipse-syson/syson/issues/2200[#2200] [diagrams] Remove the _expand/collapse_ tools from graphical nodes to fix a bug caused by their interaction with the _Manage Visibility menu_.
This menu supersedes almost all the tools' behavior, with the minor exception that the _expand_ tool restored the visibility state of all child graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2507[#2507] [explorer] Fix the missing _New objects from text_ _Explorer_ view palette tool when filtered by the palette search.
- https://github.com/eclipse-syson/syson/issues/2507[#2507] [explorer] Highlight _Create Expression_, _Edit Expression_, _Delete Expression_, and _New objects from text_ _Explorer_ view and diagram palette tools when they are filtered by the palette search.

=== Improvements

- https://github.com/eclipse-syson/syson/issues/247[#247] [details] Hide `Memberships` and root `Namespaces`` in Reference Widget model-browser dialogs, making models and standard libraries easier to navigate.
- https://github.com/eclipse-syson/syson/issues/2450[#2450] [diagrams] Apply the default _Explorer_ filters to selection dialogs used by node tools.
Memberships, root namespaces, expose elements, and expression internals are hidden, while library elements remain available for selection.
...
- https://github.com/eclipse-syson/syson/issues/2480[#2480] [diagrams] Add the _Delete from diagram_ action to the group palette in standard diagrams to support multi-selection.
- https://github.com/eclipse-syson/syson/issues/2468[#2468] [diagrams] Remove expression syntax from the direct-edit grammar.
Direct edit now only updates labels and feature syntax; use the dedicated expression tools to create or modify expressions.
- [cleanup] Retrieve the label of _Create Expression_, _Edit Expression_ and _Delete Expression_ tree palette tools from the backend instead of using a frontend hard-coded value.
...
- https://github.com/eclipse-syson/syson/issues/2370[#2370] [diagrams] Add support for `ItemUsage` inheritance in _items_ compartments of `PortUsage` and `PortDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2372[#2372] [diagrams] Add support for assume constraint inheritance in _assume constraints_ compartments of `RequirementUsage` and `RequirementDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2375[#2375] [diagrams] Add support for require constraints inheritance in _require constraints_ compartments of `RequirementUsage`, `RequirementDefinition` and subtypes graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2378[#2378] [diagrams] Add support for framed concern inheritance in frames compartment of RequirementUsage and RequirementDefinition.
- https://github.com/eclipse-syson/syson/issues/2380[#2380] [diagrams] Add support for `SatisfyRequirementUsage` inheritance in _satisfy requirements_ compartment of `PartUsage` and `PartDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2382[#2382] [diagrams] Add support for stakeholder inheritance in _stakeholders_ compartments of `RequirementUsage` and `RequirementDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2387[#2387] [diagrams] Add support for objective requirement inheritance in _objective_ compartments of `CaseUsage` and `CaseDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2391[#2391] [diagrams] Add support for actor inheritance in _actors_ compartment of `CaseDefinition` and `CaseUsage` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2389[#2389] [diagrams] Add support for inheritance of subject in _subject_ compartment of `CaseDefinition` and `CaseUsage` (and more) graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2402[#2402] [diagrams] Add support for the inheritance of the parameter border nodes of `ActionDefinition` and `ActionUsage` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2409[#2409] [diagrams] Add _requirements_ compartments to `PartUsage` and `PartDefinition` graphical nodes.
It is also now possible to create new requirements using the _New Requirement_ tool from the `PartUsage` and `PartDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2410[#2410] [diagrams] Add _concerns_ compartments to `PartUsage` and `PartDefinition` graphical nodes.
It is also now possible to create new concerns using the _New Concern_ tool from the `PartUsage` and `PartDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/2397[#2397] [diagrams] Display existing binary requirement derivations as edges in the _General View_.
...
- [releng] Update to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2026.7.0].
...
- https://github.com/eclipse-syson/syson/issues/2329[#2329] [import] Fix importing requirement (and sub-requirement) a text.
- https://github.com/eclipse-syson/syson/issues/2336[#2336] [details] Fix _Edit expression_ modal when the selected element is a `FeatureValue`
...
- https://github.com/eclipse-syson/syson/issues/2166[#2166] [cleanup] Remove unused `ViewToolService#getParentViewExpression` services.
...
- [releng] Update to https://github.com/spring-projects/spring-boot/releases/tag/v4.0.6[Spring Boot 4.0.6].
...
- https://github.com/eclipse-sirius/sirius-web/issues/5941[#5941] [explorer] As a consequence of a Sirius Web change, some SysON _Explorer_ APIs have been updated:
...
- [releng] Update to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2026.3.0]
- [releng] Update to https://github.com/spring-projects/spring-boot/releases/tag/v3.5.11[Spring Boot 3.5.11]
...
- https://github.com/eclipse-syson/syson/issues/2023[#2023] [diagrams] On diagrams, `ConnectionDefinition` graphical nodes are now correctly labelled as `«connection def»`.
...
- https://github.com/eclipse-syson/syson/issues/1943[#1943] [diagrams] Add _attributes_ and _doc_ compartments on `ConnectionDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/1946[#1946] [diagrams] Add _actors_ compartment on `RequirementUsage` and `RequirementDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/1947[#1947] [diagrams] Add _subjects_ as graphical nodes in addition to the existing the _subject_ compartment for `CaseUsages`, `CaseDefinitions`, `UseCaseUsage`, `UseCaseDefinitions`,  `RequirementsUsages`, `RequirementsDefinitions` and `SatisfyRequirementUsages`.
- https://github.com/eclipse-syson/syson/issues/1954[#1954] [export] Implement textual export of `TransitionUsage`.
- https://github.com/eclipse-syson/syson/issues/1966[#1966] [export] Implement textual export of `ExhibitStateUsage`.
- https://github.com/eclipse-syson/syson/issues/1942[#1942] [diagrams] Add _parts_ compartment on `PartDefinition`, `ConnectionDefinition`, `AllocationDefinition` and `InterfaceDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/1962[#1962] [diagrams] Add graphical edge tools to add an `AttributeUsage` as a nested attribute of a `Usage` or a `Definition`.
Also, add a graphical edge tool to all `Usage` and `Definition` to make an `AttributeUsage` nested.
...
- https://github.com/eclipse-syson/syson/issues/1694[#1694] Customize Elasticsearch index to work with SysML metamodel
...
NOTE: The signature of the _reconnectSource_ and _reconnect_Target_ now takes graphical nodes as inputs.
It helps handling inherited elements that are not displayed in their semantic owner.

=== Dependency update

- [releng] Update to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2026.1.0]
...
- https://github.com/eclipse-syson/syson/issues/1698[#1698] [explorer] Discard non-`Namespace` elements to compute whether a model is a user library.
...
- https://github.com/eclipse-syson/syson/issues/1798[#1798] [explorer] Fix an issue where a document with no content was wrongly categorized as a user library.
...
- https://github.com/eclipse-syson/syson/issues/1869[#1869] [diagrams] When creating a _Do action with referenced action_ on a `StateUsage`, ensure the corresponding compartments are revealed.

=== Improvements

- https://github.com/eclipse-syson/syson/issues/1666[#1666] [export] Export `SatisfyRequirementUsage` in textual format.
- https://github.com/eclipse-syson/syson/issues/1653[#1653] [services] Improve service for creating root objects in a document so that it delegates non-SysML scenarios to the default service implementation.
- https://github.com/eclipse-syson/syson/issues/1735[#1735] [diagrams] Improve the performances of the visibility menu actions
- https://github.com/eclipse-syson/syson/issues/1702[#1702] [libraries] Provide a SysON version of the impact analysis tree.
- https://github.com/eclipse-syson/syson/issues/1746[#1746] [diagrams] When attempting to create a redundant `FeatureTyping` between elements which already have one, an informative message is displayed instead.
- https://github.com/eclipse-syson/syson/issues/1766[#1766] [diagrams] Also prevent the creation of redundant `Subsetting`, `Redefinition` and `Subclassifications`
...
- https://github.com/eclipse-syson/syson/issues/1806[#1806] [diagrams] On _Interconnection View_ diagrams, it is now possible to create and see `SatisfyRequirementUsage` graphical nodes in the _interconnection_ compartment.
- https://github.com/eclipse-syson/syson/issues/1796[#1796] [services] Remove duplicated code for diagram and `ViewUsage` creation.
- https://github.com/eclipse-syson/syson/issues/1809[#1809] [diagrams] In diagrams, it is now possible to create, edit and see `Documentation` in the _objective_ compartment of `CaseUsage`, `CaseDefinition`, `UseCaseUsage` and `UseCaseDefinition` graphical nodes.
- https://github.com/eclipse-syson/syson/issues/1820[#1820] [services] Allow _New Subject_ tool to select a `Type` instead of a `Usage`.
- https://github.com/eclipse-syson/syson/issues/1822[#1822] [services] Allow _New Actor_ tool to select `PartDefinition` in addition to `PartUsage`.
- https://github.com/eclipse-syson/syson/issues/1835[#1835] [diagrams] Align the size of _NamespaceImport_ graphical node with the size of _Package_ graphical node.
- https://github.com/eclipse-syson/syson/issues/1832[#1831] [export] Improve textual export performance by keeping a cache `Membership` computations.
...
- https://github.com/eclipse-syson/syson/issues/1773[#1773] [diagrams] Add _items_ graphical border node on `PortUsage` graphical nodes in the _General View_ diagram.
- https://github.com/eclipse-syson/syson/issues/1772[#1772] [diagrams] Add _items_ compartment on `PortUsage` graphical nodes in the _General View_ diagram.
- https://github.com/eclipse-syson/syson/issues/1770[#1770] [diagrams] Add `IDirectEditNameSpaceProvider` to provide default namespace provider, an implementation allows to look for symbols in the SI namespace.
...
- https://github.com/eclipse-syson/syson/issues/1638[#1638] [services] The following services have been moved from `LabelService` and `ViewLabelService` to `DiagramMutationLabelService`:
...
- https://github.com/eclipse-syson/syson/issues/1638[#1638] [services] The following services have been moved from `ToolService` and `ViewToolService` to `DiagramMutationExposeService`:
...
- https://github.com/eclipse-syson/syson/issues/1638[#1638] [services] The following services have been moved from `ToolService` and `ViewToolService` to `DiagramMutationDndService`:
...
- https://github.com/eclipse-syson/syson/issues/1609[#1609] [services] Update the explorer services to match the new infrastructure.
...
- https://github.com/eclipse-syson/syson/issues/1166[#1166] [explorer] Fix library workbench to always displays the current library at the root of the explorer.
...
- [explorer] _NEW_OBJECTS_FROM_TEXT_MENU_ENTRY_CONTRIBUTION_ID_ constant has been moved from `org.eclipse.syson.tree.explorer.view.SysONExplorerTreeDescriptionProvider` to `org.eclipse.syson.tree.explorer.view.menu.context.SysONExplorerTreeItemContextMenuEntryProvider`.
`SysONDocumentTreeItemContextMenuContribution` and `SysONObjectTreeItemContextMenuContribution` frontend components have been deleted.
SysON now uses the Sirius-Web implementations of these components.

=== Dependency update

- [releng] Update to ANTLR 4.13.2 to match the version sirius-web is using
- [releng] Update to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2025.12.0]
...
- https://github.com/eclipse-syson/syson/issues/1640[#1640] [export] Properly handle `UnaryOperator` during textual export.
- https://github.com/eclipse-syson/syson/issues/1643[#1643] [export] Textual export fails to serialize a reference to an `Element` having only a _shortName_ (no _declaredName_).
- https://github.com/eclipse-syson/syson/issues/1166[#1166] [explorer] Fix library workbench to always displays the current library at the root of the explorer.
Libraries containing only `LibraryPackage` instances were previously displayed under the `User libraries` directory.
- https://github.com/eclipse-syson/syson/issues/1664[#1664] [diagrams] Fix an issue where the creation of some elements inside a `ViewUsage` graphical node was creating the element at the root of the diagram instead of inside the `ViewUsage` graphical node.
- https://github.com/eclipse-syson/syson/issues/1679[#1679] [validation] Fix AQL errors on several validation rules.
- https://github.com/eclipse-syson/syson/issues/1680[#1680] [explorer] Fix an error where the creation of a `ViewUsage` from the _Explorer_ view was not typing the `ViewUsage` with the selected `ViewDefinition`.
- https://github.com/eclipse-syson/syson/issues/1683[#1683] [diagrams] Fix an error where the creation of a `NamespaceImport` graphical node as first element in a diagram was not hiding "Add your first element" graphical node.
- https://github.com/eclipse-syson/syson/issues/1688[#1688] [details] Fix an error where the _reference widget_ 's object selection popup was switching from many-valuated to mono-valuated UI when selecting an element on the _Typed By_ property on `Feature` elements.
...
- https://github.com/eclipse-syson/syson/issues/1642[#1642] [export] Export `ConnectionUsage` in textual format.
- https://github.com/eclipse-syson/syson/issues/1609[#1609] [explorer] Rework how the explorer displays libraries.
The explorer now displays imported-by-reference libraries that only contain `LibraryPackage` in the `User libraries` directory.
Other libraries are always displayed at the root of the explorer.
The explorer now appends `[read-only]` to read-only models and the `KerML` and `SysML` directories.
The explorer now appends `library-name@library-version` to imported-by-reference library models.
- https://github.com/eclipse-syson/syson/issues/1661[#1661] [frontend] Extract the frontend extension registry in a separate file and move it from _syson_ to _syson-components_ frontend module in order to be easily used by downstream applications.
- https://github.com/eclipse-syson/syson/issues/1681[#1681] [diagrams] Improve the way _Elements_ are displayed in the _General View_ diagram by not display them twice. The new behavior is the following: by default, when no compartment is displayed inside a graphical node, the creation of a nested _Element_ from the graphical node displays it on the diagram background and links it with the graphical node through a composition graphical edge. Then, when any compartment inside a graphical node _A_ containing a nested graphical node _B_ is displayed, then any graphical node representing _B_ and linked to graphical node _A_ through a composition graphical edge disappear.
...
In addition, two new tools are added to the palette of the _General View_ diagram:
...
- https://github.com/eclipse-syson/syson/issues/1684[#1684] [diagrams] When a `ViewUsage` is created from any standard diagram, then a diagram is now created and associated to this `ViewUsage`.
- https://github.com/eclipse-syson/syson/issues/1682[#1682] [explorer] Allow the creation of `InterconnectionView` diagrams from _Packages_ elements.
- https://github.com/eclipse-syson/syson/issues/1685[#1685] [explorer] Forbid the creation of a standard diagram on a `ViewUsage` with a standard diagram.
Remove the Delete tool from the context menu of a standard diagram.
Users will have to delete the `ViewUsage` from the _Explorer_ view to delete the associated diagram.
Remove the Duplicate tool from the context menu of a standard diagram.
- https://github.com/eclipse-syson/syson/issues/1686[#1686] [diagrams] Add _New Documentation_ tool on `Package` graphical nodes in standard diagrams.
- https://github.com/eclipse-syson/syson/issues/1689[#1689] [diagrams] Do not display _ports_ compartment when a _PortUsage_ is created from any graphical node in standard diagrams.
...
- https://github.com/eclipse-syson/syson/issues/1638[#1638] [diagrams] Add the capability to put an `Element` graphical node inside a `ViewUsage` in diagrams
- https://github.com/eclipse-syson/syson/issues/1584[#1584] [diagram] Add a "parts" compartment in diagrams (hidden by default)
...
- [releng] Update to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2025.10.0]
...
- https://github.com/eclipse-syson/syson/issues/1169[#1169] [syson] Add a new navigation bar menu icon on the top right corner of the application.
...
- https://github.com/eclipse-syson/syson/issues/433[#433] [tests] Add scripts to setup and update the integration testing environment
