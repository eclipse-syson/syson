...
- _ImplicitSpecializationSwitch_ now requires an _ILibraryNamespaceProvider_ to access element from `LibraryPackage`. A default implementation is given with _org.eclipse.syson.sysml.helper.ImplicitSpecializationSwitch_.
...
- https://github.com/eclipse-syson/syson/issues/1422[#1422] [releng] The following files have been move to `syson-components` and it is no longer necessary to add them to a project on top of SysON:
...
- [releng] Switch to https://github.com/eclipse-sirius/sirius-web[Sirius Web 2025.6.10]
