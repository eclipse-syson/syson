...
- https://github.com/eclipse-syson/syson/issues/1962[#1962] [diagrams] Add edge tools to add an `AttributeUsage` as a nested attribute of a `Usage` or a `Definition`.
Also, add an edge tool to all `Usage` and `Definition` to make an `AttributeUsage` nested.
