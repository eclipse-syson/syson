...
/**
 * Provides the palette for the SysON Explorer view.
 *
 * @author flatombe
 */
@Service
public class SysONExplorerTreeItemContextMenuEntryProvider implements ITreeItemPaletteCustomizer {
    ...
    public Palette customize(IEditingContext editingContext, TreeDescription treeDescription, Tree tree, TreeItem treeItem, Palette palette) {
        return this.getPalette(editingContext, treeDescription, tree, treeItem);
    }
    ...
    public Palette getPalette(IEditingContext editingContext, TreeDescription treeDescription, Tree tree, TreeItem treeItem) {
        ...
        if (this.sysonExplorerService.canExpandAll(treeItem, editingContext)) {
            result.add(new SingleClickTreeItemTool(ExplorerTreeItemContextMenuEntryProvider.EXPAND_ALL, this.messageService.treeToolExpandAll(), List.of(), false, List.of()));
        }
        return new Palette("", List.of(), result);
    }
    ...
    private List<IPaletteEntry> getObjectContextMenuEntries(IEMFEditingContext editingContext, TreeItem treeItem) {
        ...
        if (optionalEObject.isPresent()) {
            ...
            if (this.sysonExplorerService.isEditable(object)) {
                entries.add(new SingleClickTreeItemTool(ExplorerTreeItemContextMenuEntryProvider.NEW_OBJECT, this.messageService.treeToolNewObject(), List.of(), false, List.of()));
                ...
                if (object instanceof Element && !(object instanceof Relationship)) {
                    entries.add(new SingleClickTreeItemTool(ExplorerTreeItemContextMenuEntryProvider.DUPLICATE_OBJECT, this.messageService.treeToolDuplicateObject(), List.of(), false,
                            List.of()));
                }
            }
    ...
    private List<IPaletteEntry> getRepresentationContextMenuEntries(IEMFEditingContext editingContext, TreeItem treeItem) {
        ...
        if (optionalRepresentationMetadata.isPresent()) {
            ...
            if (!SysONRepresentationDescriptionIdentifiers.GENERAL_VIEW_DIAGRAM_DESCRIPTION_ID.equals(representationMetadata.getDescriptionId())
                ...
                return List.of(
                        new SingleClickTreeItemTool(ExplorerTreeItemContextMenuEntryProvider.DUPLICATE_REPRESENTATION, this.messageService.treeToolDuplicateRepresentation(), List.of(),
                                false, List.of()));
            }
        }
        return List.of();
