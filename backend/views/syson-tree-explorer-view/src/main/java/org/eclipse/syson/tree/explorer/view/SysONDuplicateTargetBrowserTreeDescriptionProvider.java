...
public class SysONDuplicateTargetBrowserTreeDescriptionProvider implements IEditingContextRepresentationDescriptionProvider {
    ...
    private Object getParentObject(VariableManager variableManager) {
        ...
        if (self instanceof Element element) {
            OwningMembership owningMemberElement = element.getOwningMembership();
            if (owningMemberElement != null) {
                result = owningMemberElement.eContainer();
            }
        }

        if (result == null && self instanceof Namespace && ((Namespace) self).eContainer() == null) {
            result = ((Namespace) self).eResource();
        }
    ...
    private Boolean isSelectableProvider(VariableManager variableManager, IEditingContext editingContext) {
        Object targetContainer = variableManager.getVariables().get(VariableManager.SELF);

        String representationId = variableManager.get(GetOrCreateRandomIdProvider.PREVIOUS_REPRESENTATION_ID, String.class).orElse(MODEL_BROWSER_CONTAINER_PREFIX);
        List<String> ownerId = this.urlParser.getParameterValues(representationId).get("ownerId");
