...
/**
 * Used to create the edge description between elements and their nested stakeholders in General View.
 *
 * @author pcdavid
 */
public class NestedStakeholderEdgeDescriptionProvider extends AbstractEdgeDescriptionProvider {
    ...
    public NestedStakeholderEdgeDescriptionProvider(EClass eClass, EReference eReference, IColorProvider colorProvider, IDescriptionNameGenerator nameGenerator) {
        super(colorProvider);
        this.nameGenerator = Objects.requireNonNull(nameGenerator);
        this.eClass = Objects.requireNonNull(eClass);
        this.eReference = Objects.requireNonNull(eReference);
        this.edgeName = this.nameGenerator.getEdgeName("Nested Stakeholder " + this.eClass.getName());
    }
    ...
    public void link(DiagramDescription diagramDescription, IViewDiagramElementFinder cache) {
        ...
        var optStakeholderNodeDescription = cache.getNodeDescription(StakeholderNodeDescriptionProvider.NAME);
        var sourceNodes = new ArrayList<NodeDescription>();

        if (optEdgeDescription.isPresent() && optStakeholderNodeDescription.isPresent()) {
            ...
            edgeDescription.getTargetDescriptions().add(optStakeholderNodeDescription.get());
    ...
    protected boolean isDeletable() {
        // This edge cannot be deleted. This would mean that the existing stakeholder is not contained in an
        // Requirement anymore, and should likely be represented as a regular part, which is not possible out of
        // the box.
        return false;
    }
    ...
    protected ChangeContextBuilder getSourceReconnectToolBody() {
        return this.viewBuilderHelper.newChangeContext()
                .expression(ServiceMethod.of2(DiagramMutationAQLService::reconnectSourceNestedStakeholderEdge).aql(AQLConstants.EDGE_SEMANTIC_ELEMENT, AQLConstants.SEMANTIC_RECONNECTION_TARGET,
