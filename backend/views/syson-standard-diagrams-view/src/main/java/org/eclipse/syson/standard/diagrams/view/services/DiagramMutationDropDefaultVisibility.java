...
public class DiagramMutationDropDefaultVisibility implements IDiagramEventConsumer {
    ...
    public void accept(IEditingContext editingContext, Diagram previousDiagram, List<IDiagramEvent> diagramEvents, List<ViewDeletionRequest> viewDeletionRequests, List<ViewCreationRequest> viewCreationRequests, ChangeDescription changeDescription) {
        if (changeDescription.getInput() instanceof DropOnDiagramInput input && editingContext instanceof EditingContext siriusEditingContext && siriusEditingContext.getViewConversionData().get(previousDiagram.getDescriptionId()) instanceof ViewDiagramConversionData viewDiagramConversionData) {
            ...
            input.objectIds().forEach(objectId -> {
                ...
                if (semanticParent.isPresent() && semanticParent.get() instanceof Element element) {
                    ...
                    if (optionalParentNodeDescriptionId.isPresent()) {
                        ...
                        parentNodeDescription.ifPresent(nodeDescription -> nodeDescription.getReusedChildNodeDescriptionIds()
                                .stream().filter(reusedChildNodeDescriptionId -> interConnectionCompartment.isEmpty() || !interConnectionCompartment.get().equals(reusedChildNodeDescriptionId))
