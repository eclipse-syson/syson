...
public class RequirementDefinitionSubjectCompartmentNodeDescriptionProvider extends AbstractCompartmentNodeDescriptionProvider {
    ...
    public RequirementDefinitionSubjectCompartmentNodeDescriptionProvider(IColorProvider colorProvider, IDescriptionNameGenerator descriptionNameGenerator) {
        this(SysmlPackage.eINSTANCE.getRequirementDefinition(), SysmlPackage.eINSTANCE.getRequirementDefinition_SubjectParameter(), colorProvider, descriptionNameGenerator);
    }

    public RequirementDefinitionSubjectCompartmentNodeDescriptionProvider(EClass eClass, EReference eReference, IColorProvider colorProvider, IDescriptionNameGenerator descriptionNameGenerator) {
    ...
    protected List<NodeDescription> getDroppableNodes(IViewDiagramElementFinder cache) {
        ...
        cache.getNodeDescription(
                this.getDescriptionNameGenerator().getCompartmentItemName(SysmlPackage.eINSTANCE.getRequirementDefinition(), SysmlPackage.eINSTANCE.getRequirementDefinition_SubjectParameter()))
                .ifPresent(droppableNodes::add);
        cache.getNodeDescription(
    ...
    protected String getDropElementFromDiagramExpression() {
        return ServiceMethod.of6(DiagramMutationAQLService::dropSubjectFromDiagram).aqlArrow("droppedElements", "droppedNodes", "targetElement", "targetNode", CoreVariables.EDITING_CONTEXT.name(),
