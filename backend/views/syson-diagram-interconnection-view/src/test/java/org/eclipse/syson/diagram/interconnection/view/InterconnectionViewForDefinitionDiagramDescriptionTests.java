...
/**
 * Checks the structure of the Interconnection View diagram on a Definition.
 *
 * @author gdaniel
 */
public class InterconnectionViewForDefinitionDiagramDescriptionTests {

    private List<Class<?>> diagramServices;

    private DiagramDescription diagramDescription;

    private final DescriptionNameGenerator descriptionNameGenerator = new IVDescriptionNameGenerator();

    private final DiagramPredicates diagramPredicates = new DiagramPredicates("IV", this.descriptionNameGenerator);
    ...
    public void setUp() {
        ...
        IRepresentationDescriptionProvider representationDescriptionProvider = new InterconnectionViewForDefinitionDiagramDescriptionProvider();
    ...
    public void eachNodeHasDiagramPrefix() {
        ...
        new NodeDescriptionHasNameChecker().withExpectedPrefix("IV").checkAll(nodeDescriptionCandidates);
    }
    ...
    public void eachNodeHasDeleteTool() {
        Collection<NodeDescription> nodeDescriptionCandidates = EMFUtils.allContainedObjectOfType(this.diagramDescription, NodeDescription.class)
                .filter(this.diagramPredicates.isFakeNode().negate()
                        ...
                        .and(this.diagramPredicates.isCompartmentNode().negate())
                        .and(this.diagramPredicates.isInheritedCompartmentItemNode().negate()))
                // The only definition in the diagram is the root of the diagram, and it isn't allowed to delete it.
                .filter(this.diagramPredicates.hasName(this.descriptionNameGenerator.getNodeName(SysmlPackage.eINSTANCE.getDefinition())).negate())
                .toList();
