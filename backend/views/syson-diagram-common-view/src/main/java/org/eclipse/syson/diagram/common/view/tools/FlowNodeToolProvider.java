...
public class FlowNodeToolProvider implements INodeToolProvider {
    ...
    private SelectionDialogDescription getSelectionDialogDescription() {
        ...
        var selectionDialogTree = this.diagramBuilderHelper.newSelectionDialogTreeDescription()
                .elementsExpression(ServiceMethod.of1(TreeQueryAQLService::getSelectionDialogElements).aql(CoreVariables.EDITING_CONTEXT.name(), "Sequence{" + domainType + "}"))
                .childrenExpression(ServiceMethod.of3(TreeQueryAQLService::getSelectionDialogChildren).aqlSelf(CoreVariables.EDITING_CONTEXT.name(), TreeRenderer.EXPANDED, "Sequence{" + domainType + "}"))
