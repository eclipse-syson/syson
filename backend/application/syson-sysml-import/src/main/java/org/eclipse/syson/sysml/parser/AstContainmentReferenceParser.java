...
public class AstContainmentReferenceParser {
    ...
    public static void populateContainmentReference(EObject eObject, JsonNode astJson) {
        List<JsonNode> ownedJson = getOwnedJsonNode(astJson);
        List<EObject> ownedObject = ownedJson.stream().flatMap(t -> {
            return AstTreeParser.parseJsonNode(t).stream();
        }).toList();

        ownedObject.stream().forEach(t -> AstContainmentReferenceParser.ownObject(eObject, t));
        
    }


    private static List<JsonNode> getOwnedJsonNode(JsonNode astJson) {
        return ReferenceHelper.extractOwnedObject(astJson);
    }

    public static void ownObject(EObject owner, EObject owned) {
        
        /// Owning Relationship ?s
