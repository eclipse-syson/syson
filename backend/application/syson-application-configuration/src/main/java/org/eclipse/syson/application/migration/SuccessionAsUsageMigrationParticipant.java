...
/**
 * Migration participant used to migrate old SysML mode previous to 2025.4.0.
 *
 * <p>
...
public class SuccessionAsUsageMigrationParticipant implements IMigrationParticipant {
    ...
    private static final String PARTICIPANT_VERSION = "2025.4.0-202502191000";
    ...
    public void preDeserialization(JsonResource resource, JsonObject jsonObject) {
        if (this.isSysMLResource(jsonObject)) {
            this.collectElementsOfType(jsonObject, SysmlPackage.eINSTANCE.getConnectionUsage()).forEach(successionNode -> {
