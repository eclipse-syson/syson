...
public class SemanticCheckerService {
    ...
    public SemanticCheckerService(SemanticRunnableFactory semanticRunnableFactory, IObjectService objectService, String editingContextId) {
    ...
    public <T extends Element> void checkElement(Step<DiagramRefreshedEventPayload> verifier, Class<T> type, Supplier<String> idSupplier, Consumer<T> semanticChecker) {
        ISemanticChecker checker = editingContext -> {
            Optional<Object> optElement = this.objectService.getObject(editingContext, idSupplier.get());
