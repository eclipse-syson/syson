...
public class ControlNodesInOldDiagramsTest extends AbstractIntegrationTests {
    ...
    private Flux<DiagramRefreshedEventPayload> givenSubscriptionToGVDiagram() {
        var diagramEventInput = new DiagramEventInput(UUID.randomUUID(), ControlNodesInOldDiagramsTestProjectData.EDITING_CONTEXT_ID, ControlNodesInOldDiagramsTestProjectData.GraphicalIds.GV_DIAGRAM_ID);
        return this.givenDiagramSubscription.subscribe(diagramEventInput);
    }

    private Flux<DiagramRefreshedEventPayload> givenSubscriptionToAFVDiagram() {
        var diagramEventInput = new DiagramEventInput(UUID.randomUUID(), ControlNodesInOldDiagramsTestProjectData.EDITING_CONTEXT_ID, ControlNodesInOldDiagramsTestProjectData.GraphicalIds.AFV_DIAGRAM_ID);
        return this.givenDiagramSubscription.subscribe(diagramEventInput);
    }
    ...
    @GivenSysONServer({ ControlNodesInOldDiagramsTestProjectData.SCRIPT_PATH })
    @Test
    public void givenAProjectWithAGVDiagramContainingControlNodesWhenTheModelIsLoadedThenNewExtraControlNodesAreHidden() {
        ...
        Consumer<Object> initialDiagramContentConsumer = assertRefreshedDiagramThat(diagram -> {
            ...
            var rootActionNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.ROOT_ACT_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var subActionNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.SUB_ACT_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var subDecisionNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.DECISION_NODE_DEC_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var subForkNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.FORK_NODE_FOR_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var subJoinNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.JOIN_NODE_JOI_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var subMergeNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.MERGE_NODE_MER_ID.equals(node.getTargetObjectId()))
                    .findFirst();
    ...
    public void givenAProjectWithAAFVDiagramContainingControlNodesWhenTheModelIsLoadedThenNewExtraControlNodesAreHidden() {
        ...
        Consumer<Object> initialDiagramContentConsumer = assertRefreshedDiagramThat(diagram -> {
            ...
            var avfActionNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.AVF_ACTION_ACT_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var afvDecisionNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.AFV_DECISION_DEC_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var afvForkNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.AFV_FORK_FOR_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var afvJoinNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.AFV_JOIN_JOI_ID.equals(node.getTargetObjectId()))
                    .findFirst();
            var afvMergeNode = rootNodes.stream()
                    .filter(node -> ControlNodesInOldDiagramsTestProjectData.SemanticIds.AFV_MERGE_MER_ID.equals(node.getTargetObjectId()))
                    .findFirst();
