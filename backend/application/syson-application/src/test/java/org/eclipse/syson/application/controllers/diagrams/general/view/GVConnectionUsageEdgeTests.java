...
public class GVConnectionUsageEdgeTests extends AbstractIntegrationTests {
    ...
    public void createConnectionUsageOnParts() {
        ...
        Runnable checker = this.createCheckerBuilder()
                ...
                .build(newEdge, EDITING_CONTEXT_ID);
    ...
    public void directEditFeatureTyping() {
        ...
        Runnable requestDirectEdit = () -> {
            ...
            final String labelId = diagramNavigator.edgeWithId(EdgeConnectionUsageTestProjectData.GraphicalIds.CONNECTION_0_ID).getEdge().getCenterLabel().id();

            EditLabelInput input = new EditLabelInput(UUID.randomUUID(), EDITING_CONTEXT_ID, diagram.get().getId(), labelId, "connection0 : ConnectionDefinition1");
        ...
        Runnable runnableChecker = this.semanticRunnableFactory.createRunnable(EDITING_CONTEXT_ID,
                (editingContext, executeEditingContextFunctionInput) -> {
                    Optional<Object> optElement = this.objectSearchService.getObject(editingContext, EdgeConnectionUsageTestProjectData.SemanticIds.CONNECTION_0_ID);
                    assertThat(optElement).isPresent();
                    Object element = optElement.get();
                    assertThat(element).isInstanceOf(ConnectionUsage.class);
                    ConnectionUsage castedElement = (ConnectionUsage) element;
                    assertThat(castedElement.getType()).hasSize(2)
    ...
    private ConnectorAsUsageCheckerBuilder<ConnectionUsage> createCheckerBuilder() {
        return new ConnectorAsUsageCheckerBuilder<>(this.identityService, ConnectionUsage.class, this.semanticRunnableFactory, this.objectSearchService);
    }
