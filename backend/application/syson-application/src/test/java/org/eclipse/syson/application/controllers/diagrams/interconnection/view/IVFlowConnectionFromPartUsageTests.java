...
import org.eclipse.syson.application.data.InterconnectionViewIdentifiers;
...
public class IVFlowConnectionFromPartUsageTests extends AbstractIntegrationTests {
    ...
    public void setUp() {
        ...
        var diagramEventInput = new DiagramEventInput(UUID.randomUUID(),
                SysMLv2Identifiers.INTERCONNECTION_VIEW_WITH_FLOW_CONNECTION_CONTEXT_ID,
                SysMLv2Identifiers.INTERCONNECTION_VIEW_WITH_FLOW_CONNECTION_DIAGRAM);
    ...
    public void reconnectFlowConnectionUsageTarget() {
        this.verifier.then(() -> this.edgeReconnectionTester.reconnectEdge(InterconnectionViewIdentifiers.EDITING_CONTEXT_ID,
                this.diagram,
                InterconnectionViewIdentifiers.Diagram.FLOW_CONNECTION_P1_P2_ID,
                InterconnectionViewIdentifiers.Diagram.PORT_PART_3_ID,
        ...
        IDiagramChecker diagramCheckerTarget = (initialDiagram, newDiagram) -> {
            ...
            DiagramAssertions.assertThat(newEdge).hasSourceId(InterconnectionViewIdentifiers.Diagram.PORT_PART_1_ID);
            DiagramAssertions.assertThat(newEdge).hasTargetId(InterconnectionViewIdentifiers.Diagram.PORT_PART_3_ID);
        ...
        this.semanticCheckerService.checkElement(this.verifier, FlowConnectionUsage.class, () -> InterconnectionViewIdentifiers.Semantic.FLOW_CONNECTION_P1_P2_ID, flowConnectionUsage -> {
            assertThat(this.identityService.getId(flowConnectionUsage.getSource().get(0))).isEqualTo(InterconnectionViewIdentifiers.Semantic.PORT_PART_1_ID);
            assertThat(this.identityService.getId(flowConnectionUsage.getTarget().get(0))).isEqualTo(InterconnectionViewIdentifiers.Semantic.PORT_PART_3_ID);
        });
    }
    ...
    public void reconnectFlowConnectionUsageSource() {
        ...
        IDiagramChecker diagramCheckerTarget = (initialDiagram, newDiagram) -> {
            ...
            Edge newEdge = this.diagramComparator.newEdges(initialDiagram, newDiagram).get(0);
            DiagramAssertions.assertThat(newEdge).hasSourceId(InterconnectionViewIdentifiers.Diagram.PORT_PART_3_ID);
            DiagramAssertions.assertThat(newEdge).hasTargetId(InterconnectionViewIdentifiers.Diagram.PORT_PART_2_ID);
            DiagramAssertions.assertThat(newEdge.getStyle()).hasTargetArrow(ArrowStyle.InputFillClosedArrow);
        };
        ...
        this.semanticCheckerService.checkElement(this.verifier, FlowConnectionUsage.class, () -> InterconnectionViewIdentifiers.Semantic.FLOW_CONNECTION_P1_P2_ID, flowConnectionUsage -> {
            assertThat(this.identityService.getId(flowConnectionUsage.getSource().get(0))).isEqualTo(InterconnectionViewIdentifiers.Semantic.PORT_PART_3_ID);
            assertThat(this.identityService.getId(flowConnectionUsage.getTarget().get(0))).isEqualTo(InterconnectionViewIdentifiers.Semantic.PORT_PART_2_ID);
        });
    }
}
