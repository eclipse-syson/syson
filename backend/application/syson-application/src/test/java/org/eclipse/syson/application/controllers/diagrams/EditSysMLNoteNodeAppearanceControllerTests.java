...
import org.eclipse.syson.application.nodes.dto.SysMLNoteNodeAppearanceInput;
...
/**
 * Tests for SysMLNote node appearance edition.
 *
 * @author arichard
 */
@Transactional
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class EditSysMLNoteNodeAppearanceControllerTests extends AbstractIntegrationTests {
    ...
    @Autowired
    private EditSysMLNoteNodeAppearanceMutationRunner editSysMLNoteNodeAppearanceMutationRunner;
    ...
    public void givenDiagramWhenWeEditAllItsAppearanceAndResetChangesThenTheDiagramIsProperlyUpdated() {
        ...
        Consumer<Object> initialDiagramContentConsumer = assertRefreshedDiagramThat(diagram -> {
            ...
            assertThat(diagram.getNodes())
                    .filteredOn(node -> node.getInsideLabel() != null && node.getInsideLabel().getText().equals(AllCustomNodesProjectData.Labels.NOTE))
                    .hasSize(1)
                    .allMatch(node -> node.getStyle() instanceof SysMLNoteNodeStyle)
                    .extracting(node -> (SysMLNoteNodeStyle) node.getStyle())
                    .allMatch(sysMLNoteNodeStyle -> "#ffffff".equals(sysMLNoteNodeStyle.getBackground()))
                    ...
                    .allMatch(sysMLNoteNodeStyle -> LineStyle.Solid.equals(sysMLNoteNodeStyle.getBorderStyle()));

            var siriusWebApplicationNode = new DiagramNavigator(diagram).nodeWithLabel(AllCustomNodesProjectData.Labels.NOTE).getNode();
            siriusWebApplicationNodeId.set(siriusWebApplicationNode.getId());
        });

        Runnable setNodeCustomAppearance = () -> {
            var appearanceInput = new SysMLNoteNodeAppearanceInput("red", "blue", 5, LineStyle.Dash);

            var input = new EditSysMLNoteNodeAppearanceInput(
                    UUID.randomUUID(),
            ...
            this.editSysMLNoteNodeAppearanceMutationRunner.run(input);
        };
        ...
        Consumer<Object> updatedAfterCustomAppearanceDiagramContentConsumer = assertRefreshedDiagramThat(diagram -> {
            assertThat(diagram.getNodes())
                    ...
                    .allMatch(sysMLNoteNodeStyle -> "blue".equals(sysMLNoteNodeStyle.getBorderColor()))
                    .allMatch(sysMLNoteNodeStyle -> sysMLNoteNodeStyle.getBorderSize() == 5)
                    .allMatch(sysMLNoteNodeStyle -> LineStyle.Dash.equals(sysMLNoteNodeStyle.getBorderStyle()));
        });
    ...
    public void givenDiagramWhenWeEditTwoOfItsAppearanceAndResetOneChangeThenOnlyOnePropertyIsReset() {
        ...
        Runnable setNodeCustomAppearance = () -> {
            var appearanceInput = new SysMLNoteNodeAppearanceInput("red", null, 5, null);
