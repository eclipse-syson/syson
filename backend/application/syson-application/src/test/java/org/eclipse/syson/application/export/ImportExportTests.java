...
public class ImportExportTests extends AbstractIntegrationTests {
    ...
    /**
     * Test file StateTest.sysml. The content of StateTest.sysml that have been copied below
     * is under LGPL-3.0-only license. The LGPL-3.0-only license is accessible at the root of this repository, in the
     * LICENSE-LGPL file.
     *
     * <p><b>NOTE:</b> :
     * <ul>
     *     <li>The ConstructionExpression has been removed since can't be parsed with our version of the parser.</li>
     *     <li>The shorthand notation for the SendUsageAction has been removed du to <a href="https://github.com/eclipse-syson/syson/issues/1957">1957</a></li>
     * </ul> </p>
     *
     * @see <a href=
     ...
     */
    private static final String STATE_TEST_SYSML_CONTENT = """
            package StateTest {
                attribute def Sig {
                    x;
                }
    ...
    public void checkStates() throws IOException {
        var expected = """
                package StateTest {
                    ...
                    state def S {
                        ...
                        entry ;
                        state S1;
                        transition S1 accept s : Sig do action D then S2;
                        state S2 {
                            state S3;
                        }
                        transition S2 accept Exit then done;
                        transition S1 accept s : Sig do action D then S2.S3;
                        transition T first S2.S3 accept s : Sig via p if true do send s via this to p then S1;
                        exit act;
                        state S3 {
                            state S3a;
                        }
        ...
        this.checker.textToImport(STATE_TEST_SYSML_CONTENT)
                .expectedResult(expected)
                .check();
    }
