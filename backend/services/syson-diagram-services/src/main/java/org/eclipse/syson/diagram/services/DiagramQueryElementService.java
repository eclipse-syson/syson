...
public class DiagramQueryElementService {
    ...
    private boolean isValidConnectionEnd(Feature endFeature, org.eclipse.sirius.components.representations.Element nodeToTest, DiagramRenderingCache cacheRendering,
        ...
        if (endFeature != null && endFeature.getChainingFeature().size() > 1) {
            ...
            if (beforeLastChain.getOwnedFeature().contains(lastChain)) {
                // If the target is owned by previous it means this is not an inherited feature
                // Do not apply any restriction
                isValidSource = true;
            } else {
                // It is an inherited feature, the end is valid only if displayed nested in the proper graphical parent
                // Note that this approach will not play well if at some point SysOn allow displaying inherited feature at the root level (for example with a composition link)
                isValidSource = this.isDisplayIn(beforeLastChain, nodeToTest, cacheRendering, editingContext);
            }
    ...
    private boolean isDisplayIn(Feature expectedSemanticParent, org.eclipse.sirius.components.representations.Element node, DiagramRenderingCache cacheRendering, IEditingContext editingContext) {
